// @ts-check
/**
 * Optional OpenAI-compatible local supervisor for safe auto-approvals and suggest mode.
 *
 * @module supervisor
 */
...
const DEFAULT_BASE_URL = process.env.LIFTOFF_SUPERVISOR_BASE_URL
    || process.env.OPENAI_BASE_URL
    || 'http://127.0.0.1:20128/v1';
const DEFAULT_MODEL = process.env.LIFTOFF_SUPERVISOR_MODEL || 'replace-with-model-id';
const DEFAULT_API_KEY = process.env.LIFTOFF_SUPERVISOR_API_KEY
    || process.env.OPENAI_API_KEY
    || 'replace-with-local-secret';
const DEFAULT_TIMEOUT_MS = Number.parseInt(process.env.LIFTOFF_SUPERVISOR_TIMEOUT_MS || '8000', 10);
...
export class AISupervisor {
    constructor() {
        ...
        this.enabled = String(
            process.env.LIFTOFF_SUPERVISOR_ENABLED || process.env.OLLAMA_SUPERVISOR_ENABLED || ''
        ).toLowerCase() === 'true';
    ...
    async requestRemoteDecision(commandText) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Supervisor request failed with status ${response.status}${summary}`);
        }
        ...
        return {
            ...
            reason: approved ? 'supervisor-approved' : 'supervisor-denied',
            commandText
        };
    }
    ...
    async requestChatResponse(messages) {
        ...
        if (!response.ok) {
            const details = await response.text().catch(() => '');
            const summary = details ? `: ${details.slice(0, 240)}` : '';
            throw new Error(`Supervisor assist request failed with status ${response.status}${summary}`);
        }

        const result = await response.json();
        const text = extractCompletionText(result);
        if (!text) {
            throw new Error('Supervisor assist response was empty');
        }
        return text;
    }
    ...
    async chatWithUser(message, context = {}) {
        ...
        if (this.enabled) {
            try {
                ...
                const metrics = stats.metrics || {};
                const systemPrompt = [
                    'You are the LiftOff approval supervisor.',
