...
export async function discoverCDP() {
    ...
    for (const port of PORTS) {
        try {
            ...
            // Compatibility fallback for newer Antigravity builds whose main
            // workspace target no longer exposes the old workbench/jetski URLs.
            const genericPage = list.find(t => t.type === 'page' && t.webSocketDebuggerUrl);
            if (genericPage?.webSocketDebuggerUrl) {
                console.log('Found generic page target:', genericPage.title || genericPage.url || '(untitled)');
                return { port, url: genericPage.webSocketDebuggerUrl };
            }
        } catch (e) {
    ...
    for (const port of PORTS) {
        try {
            ...
            for (const t of list) {
                ...
                const isGenericPage = t.type === 'page';
                if (!isWorkbench && !isGenericPage) continue;
